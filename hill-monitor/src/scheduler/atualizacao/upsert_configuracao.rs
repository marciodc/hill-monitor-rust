...
pub async fn upsert_configuracoes(
    db: &DatabaseConnection,
    configs: &[hill_common::entity::Configuracao],
) -> Result<(), DbErr> {
    if configs.is_empty() {
        return Ok(());
    }
    ...
    for config in configs {
        ...
        configuracao::ActiveModel {
            ...
            exibir_valor_fechamento_caixa: ActiveValue::Set(
                config.exibir_valor_fechamento_caixa.clone(),
            ),
            ...
            quantidade_maxima_abastecimento: ActiveValue::Set(
                config.quantidade_maxima_abastecimento,
            ),
            ...
            imprime_gerencial_fidelidade: ActiveValue::Set(
                config.imprime_gerencial_fidelidade.clone(),
            ),
            ...
            imprime_rel_fechamento_caixa: ActiveValue::Set(
                config.imprime_rel_fechamento_caixa.clone(),
            ),
            imprime_rel_fechamento_turno: ActiveValue::Set(
                config.imprime_rel_fechamento_turno.clone(),
            ),
            ...
            abre_venda_consulta_produto: ActiveValue::Set(
                config.abre_venda_consulta_produto.clone(),
            ),
            ...
            lista_todos_abastecimentos_pdv: ActiveValue::Set(
                config.lista_todos_abastecimentos_pdv.clone(),
            ),
            id_token: ActiveValue::Set(config.id_token.clone()),
            token_csc: ActiveValue::Set(config.token_csc.clone()),
            controle_estoque_combustivel: ActiveValue::Set(
                config.controle_estoque_combustivel.clone(),
            ),
        }
        .insert(db)
        .await?;
